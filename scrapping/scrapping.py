...
def withRangeNumbers_from_string(text):
    """
    It takes a string as "Area: 60-80m²" and gives a [60, 80] string.
    """
...
def extract_info(info_type, property_html):
    """
    Checks if the text is a single number or a range and uses the correct function.
    """
...
def web_scraping():
    ...
    for i in range(1,total_pages):
        ...
        for item in properties_info:
            ...
            if multipleRentals:
                pass
            # ignores when the rental ad has mutiple rentals due the fact that in the site case whenever this happens
            # doesnt show in the html to proceed with the extraction
            else:
